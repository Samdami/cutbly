...
from .serializers import URL_CREATE_FIELD, URL_FIELDS
from ..helpers.utils import db
from helpers.url_processing import URLProcessing
from models import User
from models import Url
from auth.views import DateTimeEncoder


url_namespace = Namespace(
    "urls", "Url operations api endpoints namespace", path="/urls"
)

url_create_model = url_namespace.model("Url  Serializer", URL_CREATE_FIELD)
url_model = url_namespace.model("Url", URL_FIELDS)
...
class GenerateURLApiView(Resource):
    ...
    def post(self):
        ...
        if not auth_user:
            return {"message": "Cannot find record of this user."}, HTTPStatus.NOT_FOUND
...
class URLClickApiView(Resource):
    ...
    def get(self, url_code):
        ...
        if not url:
            return {"message": "Invalid url."}, HTTPStatus.NOT_FOUND
...
class GetLatestURLSApiView(Resource):
    ...
    def get(self):
        ...
        urls = (
            Url.query.filter_by(user_id=auth_user.id)
